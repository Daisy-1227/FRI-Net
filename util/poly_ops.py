...
def pad_gt_polys(gt_instances, num_queries_per_poly):
    ...
    for gt_inst in gt_instances:
        ...
        for i, poly in enumerate(gt_inst.gt_masks.polygons):
            corners = torch.from_numpy(poly[0]).cuda()
            corners = torch.clip(corners, 0, 255) / 255
            corner_lengths.append(len(corners))

            corners_pad = torch.zeros(num_queries_per_poly*2).cuda()
            corners_pad[:len(corners)] = corners

            labels = torch.ones(int(len(corners)/2), dtype=torch.int64).cuda()
            labels_pad = torch.zeros(num_queries_per_poly).cuda()
        ...
        room_dict = {
            ...
            'lengths': torch.tensor(corner_lengths).cuda(),
