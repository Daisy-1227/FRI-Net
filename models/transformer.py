...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, using_room_attn=False, num_queries=800, num_polys=20):
        super().__init__()
        ...
        self.using_room_attn = using_room_attn
        # room attention
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, query_pos_type='none'):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, d_model=256, n_head=8,
                 ...
                 activation="relu", num_feature_levels=4, dec_n_points=4, enc_n_points=4, query_pos_type="none"):
        super().__init__()
        ...
        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward, dropout, activation,
                                                          num_feature_levels, n_head, dec_n_points, using_room_attn=False)
        self.decoder = DeformableTransformerDecoder(decoder_layer, num_decoder_layers-1)
        
        decoder_layer_w_room_attn = DeformableTransformerDecoderLayer(d_model, dim_feedforward, dropout, activation,
                                                                      num_feature_levels, n_head, enc_n_points, using_room_attn=True)
        self.decoder.layers.append(decoder_layer_w_room_attn)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, tgt=None, tgt_masks=None):
        ...
        # decoder
        room_codes = self.decoder(tgt, reference_points, memory, src_flatten, spatial_shapes, 
                                  level_start_index, valid_ratios, query_embed, mask_flatten, tgt_masks)
